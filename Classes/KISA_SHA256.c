...
#else
	#define ROTL_ULONG(x, n) ((ULONG_SHA)((x) << (n)) | (ULONG_SHA)((x) >> (32 - (n))))
	#define ROTR_ULONG(x, n) ((ULONG_SHA)((x) >> (n)) | (ULONG_SHA)((x) << (32 - (n))))
#endif
...
#define ENDIAN_REVERSE_ULONG(dwS)	( (ROTL_ULONG((dwS),  8) & 0x00ff00ff)	\
	...
	#define BIG_B2D(B, D)		D = *(ULONG_PTR_SHA)(B)
	#define BIG_D2B(D, B)		*(ULONG_PTR_SHA)(B) = (ULONG_SHA)(D)
	#define LITTLE_B2D(B, D)	D = ENDIAN_REVERSE_ULONG(*(ULONG_PTR_SHA)(B))
	#define LITTLE_D2B(D, B)	*(ULONG_PTR_SHA)(B) = ENDIAN_REVERSE_ULONG(D)
#elif defined(LITTLE_ENDIAN)
	#define BIG_B2D(B, D)		D = ENDIAN_REVERSE_ULONG(*(ULONG_PTR_SHA)(B))
	#define BIG_D2B(D, B)		*(ULONG_PTR_SHA)(B) = ENDIAN_REVERSE_ULONG(D)
	#define LITTLE_B2D(B, D)	D = *(ULONG_PTR_SHA)(B)
	#define LITTLE_D2B(D, B)	*(ULONG_PTR_SHA)(B) = (ULONG_SHA)(D)
#else
...
// o 출력				: 
//*********************************************************************************************************************************
void SHA256_Transform(ULONG_PTR_SHA Message, ULONG_PTR_SHA ChainVar)
{
	ULONG_SHA a, b, c, d, e, f, g, h, T1, X[64];
	ULONG_SHA j;
...
void SHA256_Process(OUT SHA256_INFO *Info, IN const BYTE_SHA *pszMessage, IN UINT_SHA uDataLen)
{
	...
	{
		memcpy((UCHAR_PTR_SHA)Info->szBuffer, pszMessage, (SINT_SHA)SHA256_DIGEST_BLOCKLEN);
		SHA256_Transform((ULONG_PTR_SHA)Info->szBuffer, Info->uChainVar);
		pszMessage += SHA256_DIGEST_BLOCKLEN;
		uDataLen -= SHA256_DIGEST_BLOCKLEN;
	}

	memcpy((UCHAR_PTR_SHA)Info->szBuffer, pszMessage, uDataLen);
}
...
// o 출력				:
//*********************************************************************************************************************************
void SHA256_Close(OUT SHA256_INFO *Info, IN BYTE_SHA *pszDigest)
{
	ULONG_SHA i, Index;
	...
	{
		memset((UCHAR_PTR_SHA)Info->szBuffer + Index, 0, (SINT_SHA)(SHA256_DIGEST_BLOCKLEN - Index));
		SHA256_Transform((ULONG_PTR_SHA)Info->szBuffer, Info->uChainVar);
		memset((UCHAR_PTR_SHA)Info->szBuffer, 0, (SINT_SHA)SHA256_DIGEST_BLOCKLEN - 8);
	}
	else
		memset((UCHAR_PTR_SHA)Info->szBuffer + Index, 0, (SINT_SHA)(SHA256_DIGEST_BLOCKLEN - Index - 8));
	...
	((ULONG_PTR_SHA)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 2] = Info->uHighLength;
	((ULONG_PTR_SHA)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 1] = Info->uLowLength;
